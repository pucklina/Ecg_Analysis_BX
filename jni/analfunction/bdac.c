...
int Classify(int *newBeat,int rr, int noiseLevel, int *beatMatch, int *fidAdj,int *QrsOn,int *QrsOff,int init) ;
...
void ResetBDAC(void)
	...
	RRCount = 0 ;
	Classify(BeatBuffer,0,0,&dummy,&dummy,0,0,1) ;
	InitBeatFlag = 1 ;
...
int BeatDetectAndClassify(int ecgSample, int *beatType, int *beatMatch,int *onset,int *offset)
	...
	// Classify all other beats.
		...
		*beatType = Classify(BeatBuffer,rr,noiseEst,beatMatch,&fidAdj,&QRSonset,&QRSoffset,0) ;
		fidAdj *= SAMPLE_RATE/BEAT_SAMPLE_RATE ;
		*onset=QRSonset*2-200;
		*offset=QRSoffset*2-200;
      }
